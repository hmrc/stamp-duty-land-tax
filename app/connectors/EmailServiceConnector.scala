...
class EmailServiceConnector @Inject()(http: HttpClientV2,
  ...
  private val domain = "helpdesk@ir-efile.gov.uk"

  def submitEmailConfirmation(emailServiceRequest: EmailServiceRequest)(implicit hc: HeaderCarrier): Future[EmailServiceRequest] =
    val url: URL = url"$stubPath/$domain/email"
    http.post(url)
      .withBody(Json.toJson(emailServiceRequest))
      .execute[EmailServiceRequest]
      .recover {
        case e: Throwable =>
