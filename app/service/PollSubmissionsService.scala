...
class PollSubmissionsService @Inject() (
  ...
  private def defaultChrisUrl: String = chrisConnector.defaultPath
  ...
  private def lockRequest(sub: SubmissionForPolling, row: SelectGovTalkStatusResponse, formLockOld: String, formLockNew: String): UpdateGovTalkStatusLockRequest =
    UpdateGovTalkStatusLockRequest(
      ...
      govTalkStatus  = GovTalkStatusLock(
        ...
        gatewayUrl   = gatewayUrlOf(row).getOrElse(defaultChrisUrl)
      )
    )
  ...
  private def handlePollResponse(sub: SubmissionForPolling,
    ...
    resp match {
      case t: ChrisResponse.TransportError =>
        ...
        Future.successful(PollOutcome(sub, polled = true, pollResult = "-", newReturnStatus = sub.submissionStatus, correlationId = correlationId))

      case a: ChrisResponse.Acknowledged =>
        for {
          _ <- persistUpdate(sub, baseUpdate(fullReturn).copy(submittableStatus = Some(UniversalStatus.ACCEPTED.toString)))
  ...
  private def successOutcome(sub: SubmissionForPolling,
    ...
    for {
      _       <- persistUpdate(sub, baseUpdate(fullReturn).copy(
      ...
      _        = if (resp.utrn.isEmpty)
                   logger.warn(s"[$jobName] UC 1.44 AF11: The UTRN is not present in the Submission Response storn=${sub.storn} ref=${sub.returnResourceRef}")
      _       <- audit.auditSubmission(sub.storn, sub.returnResourceRef, correlationId, fullReturn, resp)(HeaderCarrier()).recover { case e =>
                   logger.warn(s"[$jobName] CIP audit failed (suppressed) storn=${sub.storn} ref=${sub.returnResourceRef}: ${e.getMessage}")
                 }
      _       <- emailService.submitEmailConfirmation(fullReturn, resp.utrn.getOrElse(""), None)(HeaderCarrier()).recover { case e =>
                   logger.warn(s"[$jobName] confirmation email failed (suppressed) storn=${sub.storn} ref=${sub.returnResourceRef}: ${e.getMessage}")
                 }
  ...
  private def errorOutcome(sub: SubmissionForPolling,
    ...
    if (deptError)
      logger.warn(s"[$jobName] The return is not validated by the HMRC Backend due to Business Validation Rules (BVR) Errors storn=${sub.storn} ref=${sub.returnResourceRef}")
    else
      logger.warn(s"[$jobName] The submission failed due to fatal errors from the Government Gateway storn=${sub.storn} ref=${sub.returnResourceRef}")
  ...
  private def updateStatistics(sub: SubmissionForPolling, polls: Int, pollInterval: String, gatewayUrl: Option[String]): Future[Unit] =
    chrisService.updateGovTalkStatistics(UpdateGovTalkStatisticsRequest(
      ...
      govTalkStatus  = GovTalkStatusStatistics(
        ...
        gatewayUrl           = gatewayUrl.getOrElse(defaultChrisUrl)
      )
    )).map(_ => ())
