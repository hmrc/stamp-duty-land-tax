...
class SubmissionService @Inject() (
      ...
      case Some(existing) =>
        ...
        existing.protocolStatus match
          case Some(status) if status.nonEmpty =>
            ...
            logger.info(s"[SubmissionService] RESETTING GovTalk Status formResultId=${ctx.formResultId} " +
  ...
  private def storeCorrelationId(ctx: SubmissionContext, correlationId: String)
    ...
    setGovTalkCorrelationId(ctx.storn, ctx.formResultId, correlationId).recoverWith { case e =>
      logger.error(s"[SubmissionService] could not store the GovTalk correlation id, failing the submit before sending so the return is not left unpollable returnId=${ctx.returnId} corrId=$correlationId: ${e.getMessage}")
      Future.failed(e)
    }
    ...
    adoptChrisCorrelationId(ctx, resp, correlationId).flatMap { (corrId, stored) =>
      resp match
        case c: ChrisResponse.Completed =>
          logger.info(s"[SubmissionService] handling COMPLETED returnId=${ctx.returnId} corrId=$corrId utrn=${c.utrn.getOrElse("-")} receivedIrMark=${c.receivedIrMark.getOrElse("-")}")
          successBranch(ctx, fullReturn, c, universal, seed, corrId, email)

        case a: ChrisResponse.Acknowledged =>
          logger.info(s"[SubmissionService] handling ACKNOWLEDGED returnId=${ctx.returnId} corrId=$corrId pollInterval=${a.pollIntervalSeconds.getOrElse(0)}")
          acknowledgementBranch(ctx, a, seed, corrId, stored)

        case e: ChrisResponse.Errored =>
          logger.warn(s"[SubmissionService] handling ERRORED returnId=${ctx.returnId} corrId=$corrId errorCount=${e.errors.size} numbers=${e.errors.flatMap(_.number).mkString(",")}")
          errorBranch(ctx, fullReturn, e.errors, e.responseEndPoint, universal, seed, corrId)

        case ChrisResponse.TransportError(msg, _) =>
          logger.error(s"[SubmissionService] handling TRANSPORT ERROR returnId=${ctx.returnId} corrId=$corrId: $msg")
  ...
  private def adoptChrisCorrelationId(ctx: SubmissionContext, resp: ChrisResponse, sent: String)
    ...
    assigned.map(_.trim).filter(id => id.nonEmpty && id != sent) match
      case Some(fromChris) =>
        logger.info(s"[SubmissionService] adopting the correlation id ChRIS assigned returnId=${ctx.returnId} sent=$sent assigned=$fromChris")
        setGovTalkCorrelationId(ctx.storn, ctx.formResultId, fromChris)
          .map(_ => (fromChris, true))
          .recover { case e =>
            logger.error(s"[SubmissionService] could not store the correlation id ChRIS assigned, the GovTalk row still holds the id we generated returnId=${ctx.returnId} assigned=$fromChris: ${e.getMessage}")
            (fromChris, false)
          }
      case None =>
  ...
  private def acknowledgementBranch(ctx: SubmissionContext,
    ...
    for
      ...
      _    <- if correlationIdStored then setGovTalkProtocol(ctx.storn, ctx.formResultId, "dataPoll", correlationId)
                ...
                logger.error(s"[SubmissionService] leaving the GovTalk row unpollable because the correlation id ChRIS assigned could not be stored, polling it would quote an id ChRIS does not know returnId=${ctx.returnId} corrId=$correlationId")
                Future.unit
