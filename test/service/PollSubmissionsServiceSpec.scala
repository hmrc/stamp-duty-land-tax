...
class PollSubmissionsServiceSpec extends AnyFreeSpec with Matchers with ScalaFutures with MockitoSugar {
  ...
  private trait Setup {
    ...
    def govTalkKeys: List[String] = {
      val stats: ArgumentCaptor[UpdateGovTalkStatisticsRequest] = ArgumentCaptor.forClass(classOf[UpdateGovTalkStatisticsRequest])
      verify(mockChrisService, atLeastOnce()).updateGovTalkStatistics(stats.capture())(any())
      val protocol: ArgumentCaptor[UpdateGovTalkStatusRequest] = ArgumentCaptor.forClass(classOf[UpdateGovTalkStatusRequest])
      verify(mockChrisService, atLeastOnce()).updateGovTalkStatus(protocol.capture())(any())
      val locks: ArgumentCaptor[UpdateGovTalkStatusLockRequest] = ArgumentCaptor.forClass(classOf[UpdateGovTalkStatusLockRequest])
      verify(mockChrisService, atLeastOnce()).updateGovTalkStatusLock(locks.capture())(any())
      (stats.getAllValues.asScala.toList.map(_.formResultId) ++
        protocol.getAllValues.asScala.toList.map(_.formResultId) ++
        locks.getAllValues.asScala.toList.map(_.formResultId)).distinct
    }

    def submissionKeys: List[String] = {
      val captor: ArgumentCaptor[UpdateSubmissionRequest] = ArgumentCaptor.forClass(classOf[UpdateSubmissionRequest])
      verify(mockChrisService, atLeastOnce()).updateSubmission(captor.capture())(any())
      captor.getAllValues.asScala.toList.map(_.returnResourceRef).distinct
    }
  ...
  "PollSubmissionsService" - {
    ...
    "choosing which submissions to poll" - {
      ...
      "skips a submission whose poll interval has not yet elapsed" in new Setup {
        worklist(submission)
        when(mockChrisService.selectGovTalkStatus(any())(any()))
          .thenReturn(Future.successful(govTalkRow(lastMessage = "2026-07-23 11:59:30", pollInterval = "3600")))
    ...
    "holding the GovTalk row lock while polling" - {
      ...
      "skips a submission when formp rejects the lock because the row is already held" in new Setup {
        worklist(submission)
        when(mockChrisService.updateGovTalkStatusLock(any())(any()))
      ...
      "releases the GovTalk row lock with the values read before the poll when the status cannot be re-read" in new Setup {
        worklist(submission)
        when(mockChrisService.selectGovTalkStatus(any())(any()))
          .thenReturn(Future.successful(govTalkRow(pollInterval = "300", gateway = Some("http://chris.example/poll"))))
        ...
        lockReleases.map(_.formLockOld)  must contain("Y")
        lockReleases.map(_.pollInterval) must contain only "300"
        lockReleases.map(_.gatewayUrl)   must contain only "http://chris.example/poll"
      }

      "trims the stored poll interval before sending it to formp-proxy" in new Setup {
        worklist(submission)
        when(mockChrisService.selectGovTalkStatus(any())(any()))
          .thenReturn(Future.successful(govTalkRow(pollInterval = " 60 ")))
        when(mockChrisConnector.poll(any(), any())(any()))
          .thenReturn(Future.successful(ChrisResponse.Acknowledged(Some(correlationId), None, None, "<ack/>")))

        service.invoke.futureValue mustBe Right(List(ref))

        private val captor: ArgumentCaptor[UpdateGovTalkStatusLockRequest] =
          ArgumentCaptor.forClass(classOf[UpdateGovTalkStatusLockRequest])
        verify(mockChrisService, atLeastOnce()).updateGovTalkStatusLock(captor.capture())(any())
        captor.getAllValues.asScala.toList.map(_.govTalkStatus.pollInterval) must contain only "60"
        govTalkStatistics.map(_.pollInterval) must contain only "60"
      }
    ...
    "deciding where to send the poll" - {

      "polls the stored gateway url when it already carries the ChRIS path" in new Setup {
        worklist(submission)
        when(mockChrisService.selectGovTalkStatus(any())(any()))
          .thenReturn(Future.successful(govTalkRow(gateway = Some("http://chris.example/ChRIS/SDLT/Filing/sync/SDLT"))))
        when(mockChrisConnector.poll(any(), any())(any()))
          .thenReturn(Future.successful(ChrisResponse.Acknowledged(Some(correlationId), Some(60), None, "<ack/>")))

        service.invoke.futureValue mustBe Right(List(ref))

        polledUrls mustBe List(Some("http://chris.example/ChRIS/SDLT/Filing/sync/SDLT"))
      }

      "keys GovTalk by the submission id and the submission by the return resource ref" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any())).thenReturn(Future.successful(completed(sentIrMark)))

        service.invoke.futureValue mustBe Right(List(ref))

        govTalkKeys mustBe List(submission.submissionId)
        submissionKeys mustBe List(ref)
      }
    ...
    "recording the attempt before it is sent" - {
    ...
    "when ChRIS acknowledges, it is still working" - {
    ...
    "when ChRIS completes the submission" - {
      ...
      "still sends the confirmation email and persists the UTRN when the CIP audit fails" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any())).thenReturn(Future.successful(completed(sentIrMark)))
    ...
    "when ChRIS rejects the submission" - {
      ...
      "persists DEPARTMENTAL_ERROR before sending the ChRIS delete" in new Setup {
        worklist(submission)
        ...
        private val ordered = Mockito.inOrder(mockChrisService, mockChrisConnector)
        ordered.verify(mockChrisService).updateSubmission(any())(any())
        ordered.verify(mockChrisConnector).delete(any(), any())(any())
      }
      ...
      "resets the submission to STARTED when the error is recoverable" in new Setup {
        ...
        submissionStatuses.head mustBe "FATAL_ERROR"
        submissionStatuses.last mustBe "STARTED"
        submissionUpdates.last.submissionRequestDate mustBe None
      }

      "completes the submission and still reports it as polled when ChRIS returns no UTRN" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any()))
          .thenReturn(Future.successful(ChrisResponse.Completed(None, Some(sentIrMark), Some(correlationId), None, "<ok/>")))

        service.invoke.futureValue mustBe Right(List(ref))

        submissionStatuses must contain("SUBMITTED")
        submissionUpdates.flatMap(_.utrn) mustBe empty
        verify(mockChrisConnector).delete(any(), any())(any())
      }

      "leaves a departmental error unrecoverable even when the response carries a recoverable code" in new Setup {
        worklist(submission)
        private val departmental = GovTalkError(raisedBy = "Department", number = Some("3001"), errorType = "business", text = Some("BVR failure"), location = None)
        private val recoverable  = GovTalkError(raisedBy = "Gateway", number = Some("1000"), errorType = "fatal", text = Some("try again"), location = None)
        when(mockChrisConnector.poll(any(), any())(any()))
        ...
        submissionStatuses.last mustBe "DEPARTMENTAL_ERROR"
        (submissionStatuses must not).contain("STARTED")
        submissionUpdates.last.submissionRequestDate mustBe Some("2026-07-23 10:00:00.000")
      }
    }

    "when ChRIS does not answer" - {

      "leaves the submission untouched and does not count it as polled when ChRIS gives no response" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any()))
    ...
    "keeping the internal API key off ChRIS" - {

      "sends the internal API key to formp-proxy" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any())).thenReturn(Future.successful(completed(sentIrMark)))

        service.invoke.futureValue mustBe Right(List(ref))

        private val carriers: ArgumentCaptor[HeaderCarrier] = ArgumentCaptor.forClass(classOf[HeaderCarrier])
        verify(mockChrisService, atLeastOnce()).updateGovTalkStatus(any())(carriers.capture())
        carriers.getValue.extraHeaders.map(_._1) must contain("X-API-Key")
      }
