  ...
  "PollSubmissionsService" - {
    "invoke" - {
      ...
      "marks the submission DEPARTMENTAL_ERROR and records error details on a departmental error" in new Setup {
        ...
        private val detailCaptor: ArgumentCaptor[CreateSubmissionErrorDetailRequest] =
          ArgumentCaptor.forClass(classOf[CreateSubmissionErrorDetailRequest])
        verify(mockChrisService).createSubmissionErrorDetail(detailCaptor.capture())(any())
        detailCaptor.getValue.submissionErrorDetails.errorMessage mustBe "3001: BVR failure"
        detailCaptor.getValue.submissionErrorDetails.position mustBe "1"
        verify(mockChrisConnector).delete(any(), any())(any())
      }
      ...
      "persists the UTRN and IR mark even when the CIP audit fails" in new Setup {
        ...
        private val captor: ArgumentCaptor[UpdateSubmissionRequest] = ArgumentCaptor.forClass(classOf[UpdateSubmissionRequest])
        verify(mockChrisService, atLeastOnce()).updateSubmission(captor.capture())(any())
        captor.getAllValues.asScala.toList.flatMap(_.submission.utrn) must contain("123456789MA")
        verify(mockEmailService).submitEmailConfirmation(any(), any(), any())(any())
      }
      ...
      "increments the number of polls before the poll is sent" in new Setup {
        ...
        private val captor: ArgumentCaptor[UpdateGovTalkStatisticsRequest] =
          ArgumentCaptor.forClass(classOf[UpdateGovTalkStatisticsRequest])
        verify(mockChrisService, atLeastOnce()).updateGovTalkStatistics(captor.capture())(any())
        captor.getAllValues.asScala.head.govTalkStatus.numberOfPolls mustBe "3"
      }

      "stores the new poll interval from a repeat acknowledgement" in new Setup {
        worklist(submission)
        when(mockChrisConnector.poll(any(), any())(any()))
          .thenReturn(Future.successful(ChrisResponse.Acknowledged(Some(correlationId), Some(120), Some("http://chris.example/next"), "<ack/>")))
        ...
        private val captor: ArgumentCaptor[UpdateGovTalkStatisticsRequest] =
          ArgumentCaptor.forClass(classOf[UpdateGovTalkStatisticsRequest])
        verify(mockChrisService, atLeastOnce()).updateGovTalkStatistics(captor.capture())(any())
        private val stats = captor.getAllValues.asScala.toList.map(_.govTalkStatus)
        stats.map(_.pollInterval) must contain("120")
        stats.map(_.gatewayUrl) must contain("http://chris.example/next")
      }
      ...
      "falls back to the full ChRIS submission url when no gateway url is stored or returned" in new Setup {
        ...
        private val captor: ArgumentCaptor[UpdateGovTalkStatisticsRequest] =
          ArgumentCaptor.forClass(classOf[UpdateGovTalkStatisticsRequest])
        verify(mockChrisService, atLeastOnce()).updateGovTalkStatistics(captor.capture())(any())
        captor.getAllValues.asScala.toList.map(_.govTalkStatus.gatewayUrl) must contain(
          "http://chris.example/ChRIS/SDLT/Filing/sync/SDLT"
        )
      }
