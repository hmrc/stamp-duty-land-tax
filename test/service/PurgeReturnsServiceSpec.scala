...
class PurgeReturnsServiceSpec extends AnyFreeSpec with Matchers with ScalaFutures with MockitoSugar {
  ...
  "PurgeReturnsService" - {
    "invoke" - {
      ...
      "records a return as not purged when formp-proxy responds with deleted=false" in {
        when(mockConnector.getReturnsForPurge(any())(any()))
          .thenReturn(Future.successful(ReturnsForPurgeResponse(List(submitted, started))))
        when(mockConnector.deleteReturn(eqTo(DeleteReturnRequest("STN700", "9000001")))(any()))
          .thenReturn(Future.successful(DeleteReturnResponse(deleted = true)))
        when(mockConnector.deleteReturn(eqTo(DeleteReturnRequest("STN700", "9000002")))(any()))
          .thenReturn(Future.successful(DeleteReturnResponse(deleted = false)))

        newService.invoke.futureValue mustBe Right(List("9000001"))
      }

      "requests the returns due for purge using the injected clock's date" in {
        when(mockConnector.getReturnsForPurge(any())(any()))
          .thenReturn(Future.successful(ReturnsForPurgeResponse(Nil)))

        newService.invoke.futureValue mustBe Right(Nil)

        verify(mockConnector, atLeastOnce()).getReturnsForPurge(eqTo(GetReturnsForPurgeRequest(LocalDate.parse("2026-07-05"))))(any())
      }

      "skips the run when the lock is held by another instance" in {
      ...
      "returns MongoUnlockException when the lock repository fails" in {
      ...
      "returns FailedToPurgeReturns when fetching the returns due for purge fails" in {
        val fetchFailure = new RuntimeException("formp-proxy unavailable")
        when(mockConnector.getReturnsForPurge(any())(any())).thenReturn(Future.failed(fetchFailure))

        newService.invoke.futureValue mustBe Left(FailedToPurgeReturns(fetchFailure))
      }
    }
  }
}
