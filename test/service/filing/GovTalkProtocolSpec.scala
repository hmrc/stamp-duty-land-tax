...
class GovTalkProtocolSpec extends AnyFreeSpec with Matchers with ScalaFutures with MockitoSugar {
  ...
  private class TestGovTalkProtocol(override protected val chrisService: ChrisService,
    ...
    def poll(endpoint: Option[String]): Future[ChrisResponse] = sendChrisPoll(storn, returnId, endpoint, correlationId)
    ...
    def storeCorrelationId(id: String): Future[Unit]          = setGovTalkCorrelationId(storn, returnId, id)
    def checkCorrelationId(received: Option[String]): Unit    = warnOnCorrelationIdMismatch(storn, returnId, correlationId, received)
  }
  ...
  "GovTalkProtocol" - {
    ...
    "setGovTalkCorrelationId" - {
      "writes the correlation id against the return" in new Setup {
        when(mockChrisService.updateGovTalkStatusCorrelationId(any())(any()))
          .thenReturn(Future.successful(GovTalkStatusReturn(success = true)))

        protocol.storeCorrelationId(correlationId).futureValue
        ...
        verify(mockChrisService).updateGovTalkStatusCorrelationId(captor.capture())(any())
        captor.getValue.correlationId mustBe correlationId
        captor.getValue.pollInterval mustBe 0
        captor.getValue.userIdentifier mustBe storn
        captor.getValue.formResultId mustBe returnId
      }
    }

    "warnOnCorrelationIdMismatch" - {
      "keeps the stored correlation id when ChRIS answers with a different one" in new Setup {
        protocol.checkCorrelationId(Some("SOMETHINGELSE"))

        verify(mockChrisService, org.mockito.Mockito.never()).updateGovTalkStatusCorrelationId(any())(any())
      }

      "does nothing when ChRIS echoes the correlation id or returns none" in new Setup {
        protocol.checkCorrelationId(Some(correlationId))
        protocol.checkCorrelationId(None)
    ...
    "sendChrisPoll" - {
      "returns the ChRIS response and polls the endpoint it was given" in new Setup {
        val acknowledged: ChrisResponse.Acknowledged =
          ChrisResponse.Acknowledged(Some(correlationId), Some(60), Some("http://chris.example/poll"), "<ack/>")
        when(mockChrisConnector.poll(any(), any())(any())).thenReturn(Future.successful(acknowledged))

        protocol.poll(Some("http://chris.example/poll")).futureValue mustBe acknowledged

        val endpoint: ArgumentCaptor[Option[String]] = ArgumentCaptor.forClass(classOf[Option[String]])
        verify(mockChrisConnector).poll(endpoint.capture(), eqTo(correlationId))(any())
        endpoint.getValue mustBe Some("http://chris.example/poll")
      }

      "lets the connector fall back to its default when no endpoint is stored" in new Setup {
        when(mockChrisConnector.poll(any(), any())(any()))
          .thenReturn(Future.successful(ChrisResponse.TransportError("no response", "<transport-error/>")))

        protocol.poll(None).futureValue mustBe ChrisResponse.TransportError("no response", "<transport-error/>")

        verify(mockChrisConnector).poll(eqTo(None), eqTo(correlationId))(any())
      }
    }
    ...
    "isRecoverable" - {
      ...
      "treats a 2005 raised by ChRIS as recoverable — the wire type is always fatal" in new Setup {
        protocol.recoverable(Seq(govTalkError(Some("2005")).copy(errorType = "fatal"))) mustBe true
      }
