...
class ManageAgentsControllerSpec extends SpecBase {
  ...
  "ManageAgentsController" - {
    ...
    "GET delete/predefined-agent (deletePredefinedAgent)" - {
      ...
      "returns 200 OK when service runs successfully" in new BaseSetup {
        ...
        val result: Future[Result] = controller.deletePredefinedAgent()(fakeRequest)
      ...
      "propagate UpstreamErrorResponse status & message" in new BaseSetup {
        when(mockManageAgentsService.deletePredefinedAgent(eqTo("A-123"), eqTo("B-123"))(any[HeaderCarrier]))
          .thenReturn(Future.failed(UpstreamErrorResponse("boom from upstream", BAD_GATEWAY)))

        val result: Future[Result] = controller.deletePredefinedAgent("A-123", "B-123")(fakeRequest)
