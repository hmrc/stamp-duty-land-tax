...
class TransactionReturnsControllerSpec extends SpecBase {
  ...
  "TransactionReturnsController" - {
    ...
    "POST /update-transaction (updateTransaction)" - {
      ...
      "return CREATED with update response when service returns successfully" in new BaseSetup {
        when(mockTransactionReturnsService.updateTransaction(eqTo(testUpdateTransactionRequest))(any[HeaderCarrier]))
          .thenReturn(Future.successful(testUpdateTransactionReturn))

        val result: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.toJson(testUpdateTransactionRequest)))

        status(result) mustBe CREATED
        contentAsJson(result) mustBe Json.toJson(testUpdateTransactionReturn)
        verify(mockTransactionReturnsService).updateTransaction(eqTo(testUpdateTransactionRequest))(any[HeaderCarrier])
      }

      "return BAD_REQUEST with message when given an invalid json body" in new BaseSetup {
        val result: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.obj("invalid" -> "data")))
      ...
      "return BAD_REQUEST when required fields are missing" in new BaseSetup {
        val result: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.obj()))
      ...
      "return BAD_REQUEST when storn is missing" in new BaseSetup {
        ...
        val result: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(invalidRequest))
      ...
      "return 500 Unexpected error on unknown exception" in new BaseSetup {
        when(mockTransactionReturnsService.updateTransaction(any[UpdateTransactionRequest])(any[HeaderCarrier]))
      ...
      "handle valid payload with all optional transaction fields" in new BaseSetup {
        ...
        when(mockTransactionReturnsService.updateTransaction(eqTo(completeRequest))(any[HeaderCarrier]))
          .thenReturn(Future.successful(testUpdateTransactionReturn))

        val result: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.toJson(completeRequest)))

        status(result) mustBe CREATED
        verify(mockTransactionReturnsService).updateTransaction(eqTo(completeRequest))(any[HeaderCarrier])
      }

      "handle different storn formats" in new BaseSetup {
        val request1 = testUpdateTransactionRequest.copy(storn = "STORN12345")
        val request2 = testUpdateTransactionRequest.copy(storn = "STORN-ABC-123")
        val request3 = testUpdateTransactionRequest.copy(storn = "12345678")

        when(mockTransactionReturnsService.updateTransaction(any[UpdateTransactionRequest])(any[HeaderCarrier]))
          .thenReturn(Future.successful(testUpdateTransactionReturn))

        val result1: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.toJson(request1)))
        val result2: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.toJson(request2)))
        val result3: Future[Result] = controller.updateTransaction()(fakeRequest.withBody(Json.toJson(request3)))
  ...
  private trait BaseSetup {
    val mockTransactionReturnsService: TransactionReturnsService = mock[TransactionReturnsService]
    implicit val ec: ExecutionContext = cc.executionContext
    implicit val hc: HeaderCarrier   = HeaderCarrier()
    val controller = new TransactionReturnsController(cc, mockTransactionReturnsService, fakeIdentifierAction)

    val testTransactionPayload: TransactionPayload = TransactionPayload()

    val testUpdateTransactionRequest: UpdateTransactionRequest = UpdateTransactionRequest(
      storn             = "STORN12345",
      returnResourceRef = "RRF-2024-001",
      transaction       = testTransactionPayload
    )

    val testUpdateTransactionReturn: UpdateTransactionReturn = UpdateTransactionReturn(
      updated = true
    )
  }
}