...
class TransactionReturnSpec
  ...
  private val validUpdateTransactionReturnJsonFalse =
    Json.obj("updated" -> false)
  private val updateTransactionReturnTrue =
    UpdateTransactionReturn(updated = true)
  private val updateTransactionReturnFalse =
    UpdateTransactionReturn(updated = false)
  ...
  "TransactionPayload" - {
    ...
    ".reads" - {
      ...
      "must deserialize empty JSON object as all-None payload" in {
        val result =
          Json.fromJson[TransactionPayload](Json.obj()).asEither.value
    ...
    "case class" - {
      ...
      "must support copy with modifications" in {
        val modified = minimalTransactionPayload.copy(
          claimingRelief = Some("YES"),
          totalConsider = Some("50000")
        )
      ...
      "must not be equal when fields differ" in {
        minimalTransactionPayload must not equal minimalTransactionPayload
          .copy(claimingRelief = Some("YES"))
      }
    }
  }
  ...
  "UpdateTransactionRequest" - {
    ...
    ".reads" - {
      ...
      "must deserialize valid JSON with minimal transaction" in {
        val result = Json
          .fromJson[UpdateTransactionRequest](validUpdateTransactionRequestJson)
          .asEither
          .value
      ...
      "must fail to deserialize when storn is missing" in {
        Json
          .fromJson[UpdateTransactionRequest](
            validUpdateTransactionRequestJson - "storn"
          )
          .asEither
          .isLeft mustBe true
      }
      ...
      "must fail to deserialize when returnResourceRef is missing" in {
        Json
          .fromJson[UpdateTransactionRequest](
            validUpdateTransactionRequestJson - "returnResourceRef"
          )
          .asEither
          .isLeft mustBe true
      }
      ...
      "must fail to deserialize when transaction is missing" in {
        Json
          .fromJson[UpdateTransactionRequest](
            validUpdateTransactionRequestJson - "transaction"
          )
          .asEither
          .isLeft mustBe true
      }
      ...
      "must fail to deserialize when storn has invalid type" in {
        ...
        Json
          .fromJson[UpdateTransactionRequest](json)
          .asEither
          .isLeft mustBe true
      }
    ...
    ".formats" - {
      ...
      "must round-trip with minimal request" in {
        val json = Json.toJson(updateTransactionRequest)
        val result =
          Json.fromJson[UpdateTransactionRequest](json).asEither.value
    ...
    "case class" - {
      ...
      "must not be equal when fields differ" in {
        updateTransactionRequest must not equal updateTransactionRequest
          .copy(storn = "DIFFERENT")
      }
    }
  }
  ...
  "UpdateTransactionReturn" - {
    ...
    ".reads" - {
      ...
      "must deserialize valid JSON with updated true" in {
        Json
          .fromJson[UpdateTransactionReturn](
            validUpdateTransactionReturnJsonTrue
          )
          .asEither
          .value
          .updated mustBe true
      ...
      "must deserialize valid JSON with updated false" in {
        Json
          .fromJson[UpdateTransactionReturn](
            validUpdateTransactionReturnJsonFalse
          )
          .asEither
          .value
      ...
      "must fail to deserialize when updated is missing" in {
        Json
          .fromJson[UpdateTransactionReturn](Json.obj())
          .asEither
          .isLeft mustBe true
      }
      ...
      "must fail to deserialize when updated has invalid type" in {
        Json
          .fromJson[UpdateTransactionReturn](Json.obj("updated" -> "invalid"))
          .asEither
          .isLeft mustBe true
      }
    ...
    ".writes" - {
      ...
      "must serialize with updated true" in {
        (Json.toJson(updateTransactionReturnTrue) \ "updated")
          .as[Boolean] mustBe true
      }

      "must serialize with updated false" in {
        (Json.toJson(updateTransactionReturnFalse) \ "updated")
    ...
    ".formats" - {
      ...
      "must round-trip with updated true" in {
        Json
          .fromJson[UpdateTransactionReturn](
            Json.toJson(updateTransactionReturnTrue)
          )
          .asEither
          .value mustEqual updateTransactionReturnTrue
      }

      "must round-trip with updated false" in {
        Json
          .fromJson[UpdateTransactionReturn](
            Json.toJson(updateTransactionReturnFalse)
          )
          .asEither
          .value mustEqual updateTransactionReturnFalse
      }
    }
