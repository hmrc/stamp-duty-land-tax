...
final class ReturnVersioningServiceSpec extends SpecBase {
  ...
  private def mkReturnVersionUpdateReturn(newVersion: Int = 2): ReturnVersionUpdateReturn =
  ...
  "ReturnVersioningService updateReturnVersion" - {
    ...
    "must delegate to connector (happy path)" in {
      ...
      verify(connector).updateReturnVersioning(eqTo(request))(any[HeaderCarrier])
    ...
    "must return different version numbers for different requests" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN11111", "100001", "1")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN22222", "100002", "5")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
      ...
      service.updateReturnVersion(request1).futureValue mustBe mkReturnVersionUpdateReturn(2)
      service.updateReturnVersion(request2).futureValue mustBe mkReturnVersionUpdateReturn(6)

      verify(connector).updateReturnVersioning(eqTo(request1))(any[HeaderCarrier])
      verify(connector).updateReturnVersioning(eqTo(request2))(any[HeaderCarrier])
      verifyNoMoreInteractions(connector)
    }
    ...
    "must handle version 0 to version 1" in {
      val connector                           = mock[FilingFormpProxyConnector]
      val service                             = new ReturnVersioningService(connector)
      val request: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "0")
      implicit val hc: HeaderCarrier          = HeaderCarrier()
    ...
    "must handle higher version numbers" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "5")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "10")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "99")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
      ...
      verify(connector).updateReturnVersioning(eqTo(request1))(any[HeaderCarrier])
      verify(connector).updateReturnVersioning(eqTo(request2))(any[HeaderCarrier])
      verify(connector).updateReturnVersioning(eqTo(request3))(any[HeaderCarrier])
      verifyNoMoreInteractions(connector)
    }
    ...
    "must handle different storn formats" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN12345")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN-ABC-123")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("12345678")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
    ...
    "must handle different returnResourceRef formats" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(returnResourceRef = "100001")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(returnResourceRef = "999999")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(returnResourceRef = "123")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
    ...
    "must call connector exactly once per request" in {
      ...
      verify(connector, times(1)).updateReturnVersioning(eqTo(request))(any[HeaderCarrier])
      verifyNoMoreInteractions(connector)
    }
    ...
    "must handle consecutive requests independently" in {
      ...
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN11111", "100001", "1")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN22222", "100002", "2")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN33333", "100003", "3")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
      ...
      verify(connector, times(3)).updateReturnVersioning(any[ReturnVersionUpdateRequest])(any[HeaderCarrier])
      verifyNoMoreInteractions(connector)
    }
    ...
    "must handle version conflict scenarios" in {
      val connector                           = mock[FilingFormpProxyConnector]
      val service                             = new ReturnVersioningService(connector)
      val request: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "1")
    ...
    "must handle version strings as integers" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "1")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "25")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "314")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
    ...
    "must handle version increments from same return" in {
      val connector                            = mock[FilingFormpProxyConnector]
      val service                              = new ReturnVersioningService(connector)
      val request1: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN12345", "100001", "1")
      val request2: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN12345", "100001", "2")
      val request3: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest("STORN12345", "100001", "3")
      implicit val hc: HeaderCarrier           = HeaderCarrier()
    ...
    "must handle large version numbers" in {
      val connector                           = mock[FilingFormpProxyConnector]
      val service                             = new ReturnVersioningService(connector)
      val request: ReturnVersionUpdateRequest = mkReturnVersionUpdateRequest(currentVersion = "999")
      implicit val hc: HeaderCarrier          = HeaderCarrier()
