...
final class SubmissionServiceSpec extends SpecBase {
  ...
  // Regression cover for the production incident on return 1: ChRIS accepted the return, but a
  // later persistence call (the GovTalk 'dataPoll' status update) 500'd. The cleanup datetime
  // footer was rewriting the original PENDING seed over the freshly-persisted terminal status,
  // reverting ACCEPTED/SUBMITTED/FATAL_ERROR back to PENDING and masking the real outcome.
  // These tests pin the fix: the footer must stamp onto the branch's terminal-status row (never
  // the stale seed), and must be skipped entirely on a hard mid-flight failure.
